// pages/apply/apply_service/apply_service.js
const app = getApp();
Page({
    ...
    onLoad: function (options) { //加载数据渲染页面
        // // const companyId = options.companyId;//这里的options就是上一个页面中，跳转链接后面跟的参数 xx=xxx&xx=xxxx
    ...
    getAllNews: function () {
        let that = this;
        wx.request({//已改 wx.getStorageSync('prefixURL') app.globalData.prefixURL+
            url:wx.getStorageSync('prefixURL') +'/insuranceNews/getAll', //服务器地址
            menthod: "POST", 
            header: {
            ...
            success: function (res) {
                console.info("res from getAllNews is "+JSON.stringify(res.data));
    ...
    fetchServiceData: function () {  //获取服务列表
        ...
        let allNews = wx.getStorageSync('allNews');
        const newsList = [];
        console.info("allNews=" + JSON.stringify(allNews));
        for (var i = 0; i < allNews.length; i++) {
            newsList.push({
                id:i,
                "title":allNews[i].title,//这个id并非数据库中的id而是在页面从上到下的顺序
                "releaseTime": allNews[i].releaseTime.year + "-" + allNews[i].releaseTime.monthValue + "-" + allNews[i].releaseTime.dayOfMonth
            })
        };
        ...
        setTimeout(() => {
            _this.setData({
                newsList: newsList
                // newsList: _this.data.newsList.concat(newsList)
            })
            wx.hideLoading();
        }, 1500)
    },
