...
Page({
    ...
    fetchServiceData: function () {  //获取服务列表
        ...
        console.info("allServiceList=" + JSON.stringify(allServiceList));
        console.info("companyId selected is " + companyId);
        // for (var i = (page - 1) * perpage; i < page * perpage; i++) {
        for (var i = 0; i < allServiceList.length; i++) {
            if (allServiceList[i].insuranceCompany.id != companyId) continue;
            newlist.push({
                "id": i,//这个id并非数据库中的id而是在页面从上到下的顺序
                "serviceName": allServiceList[i].serviceName,
