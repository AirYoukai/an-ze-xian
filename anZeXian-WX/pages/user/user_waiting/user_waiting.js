Page({
  ...
  getMyOrders: function () {
    let that = this;
    wx.request({//已改
      url: wx.getStorageSync('prefixURL')+'/insuranceWxpay/getOrderWaiting', //服务器地址
  menthod: "POST",
      header: {
      ...
      success: function (res) {
        wx.setStorageSync('myWaiting', res.data);
  ...
  tryPay: function (e) {
    ...
    let tmp = wx.getStorageSync('myWaiting')[i];
    wx.requestPayment({
      timeStamp: tmp.timeStamp,
      nonceStr: tmp.nonceStr,
      package: 'prepay_id=' + tmp.prepayId,
      ...
      success: function (res) {
        ...
        console.log(res);
        wx.request({//再度支付成功
          url: wx.getStorageSync('prefixURL')+'/insuranceWxpay/payAgainSucceed', //服务器地址
      menthod: "POST",
          header: {
            'content-type': 'application/json'
          },
          data: {//再次支付成功后要相应修改参数，还要插入很多张表
            wxpayId: tmp.id,
            timeStamp: tmp.timeStamp,
            nonceStr: tmp.nonceStr,
  ...
  fetchServiceData: function () {  //获取城市列表
    ...
    let myWaiting = wx.getStorageSync('myWaiting');
    console.info("myWaiting= " + JSON.stringify(myWaiting));
    const newlist = [];

    for (var i = 0; i < myWaiting.length; i++) {
      //如果当前已经超过下单2小时则不管
      if (new Date(myWaiting[i].unifiedorderTime) > new Date()) continue;
      newlist.push({
        "id": i,//这个id并非数据库中的id而是在页面从上到下的顺序
        "body": myWaiting[i].body,
        "totalFee": myWaiting[i].totalFee,
        "expireTime":
          myWaiting[i].unifiedorderTime.year + '-' + myWaiting[i].unifiedorderTime.monthValue + '-' + myWaiting[i].unifiedorderTime.dayOfMonth + ' ' +
          myWaiting[i].unifiedorderTime.hour + ':' + myWaiting[i].unifiedorderTime.minute
      });
    }
    setTimeout(() => {
      _this.setData({
