Page({
  ...
  getMyOrders: function () {
    let that = this;
    wx.request({
      url: wx.getStorageSync('prefixURL')+'/insuranceClaim/selectByOpenId', //服务器地址
   menthod: "POST",
      header: {
      ...
      success: function (res) {
        wx.setStorageSync('myClaims', res.data);
        console.info("myClaims= " + JSON.stringify(res.data));
  ...
  fetchServiceData: function () {  //获取城市列表
    ...
    let myClaims = wx.getStorageSync('myClaims');
    console.info("myClaims= " + JSON.stringify(myClaims));
    const newlist = [];

    for (var i = 0; i < myClaims.length; i++) {
      let status = "";
      if (myClaims[i].statusStaff == 'PAYEDBACK')
        status = '结束';
      else if (myClaims[i].statusManager == 'AGREED' && myClaims[i].statusStaff == 'AGREED')
        status = '待赔款';
      else if (myClaims[i].statusManager == 'DENIED' || myClaims[i].statusStaff == 'DENIED')
        status = '驳回';
      else if (myClaims[i].statusManager == 'WAITING' || myClaims[i].statusStaff == 'WAITING')
        status = '处理中';
      else
        status = '错误: 203';
      ...
      newlist.push({
        ...
        "serviceName": myClaims[i].insuranceOrder.insuranceService.serviceName,
        "companyName": myClaims[i].insuranceOrder.insuranceService.insuranceCompany.companyName,
        "status": status
      });
    }
    setTimeout(() => {
