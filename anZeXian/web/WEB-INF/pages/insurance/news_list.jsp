    ...
    <div class="main-container-inner">
        ...
        <div class="main-content">
            ...
            <div class="page-content">
                <div class="row">
                    <div class="col-md-12">
                        <h3 class="header smaller lighter blue center">新闻列表</h3>
                    </div>
                    ...
                    <div class="col-xs-3">
                        ...
                        <select id="orderByOne">
                            ...
                            <option value="short_content">摘要</option>
                            <option value="url">原文地址</option>
                    ...
                    <div class="col-md-12">
                        <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                            ...
                            <tr>
                                <td></td>
                                <td>
                                    <input class="col-xs-12" type="text" id="titleSelect">
                                </td>
                                <td>
                                    <input class="col-xs-12" type="text" id="shortContentSelect">
                                </td>

                                <td>
                                    <input class="col-xs-12" type="text" id="urlSelect">
                                </td>

                                <td>
                                    <input class="col-xs-5" type="date" id="releaseTimeFromSelect">
                                    <div class="col-xs-2">至</div>
                                    <input class="col-xs-5" type="date" id="releaseTimeToSelect">
                                </td>
                                <td>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title" id="addModalLabel">新增新闻</h4>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-sm-12">
                                    <nobr>标题</nobr>
                                    <input class="input-xxlarge" type="text" id="titleAdd">
                                </div>
                                <div class="col-sm-12">
                                    <nobr>摘要</nobr>
                                    <textarea id="shortContentAdd" class="input-xxlarge" rows="3"></textarea>
                                    <%--                                    <input class="input-xxlarge" type="text" id="shortContentAdd">--%>
                                </div>
                                <div class="col-sm-12">
                                    <nobr>原文地址</nobr>
                                    <input class="input-xxlarge" type="text" id="urlAdd">
                                </div>

                                <div class="col-sm-12">
                                    <nobr>发布时间</nobr>
                                    <input class="input-xxlarge" type="date" id="releaseTimeAdd">
                                </div>
            ...
            <div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="updateModalLabel"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                            <h4 class="modal-title" id="updateModalLabel">修改新闻</h4>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-sm-12">
                                    <nobr>标题</nobr>
                                    <input class="input-xxlarge" type="text" id="titleUpdate">
                                </div>
                                <div class="col-sm-12">
                                    <nobr>摘要</nobr>
                                    <textarea id="shortContentUpdate" class="input-xxlarge" rows="3"></textarea>
                                    <%--                                    <input class="input-xxlarge" type="text" id="shortContentUpdate">--%>
                                </div>
                                <div class="col-sm-12">
                                    <nobr>原文地址</nobr>
                                    <input class="input-xxlarge" type="text" id="urlUpdate">
                                </div>
                                <div class="col-sm-12">
                                    <nobr>发布时间</nobr>
                                    <input class="input-xxlarge" type="date" id="releaseTimeUpdate">
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
...
<script>
    $(function () {
        ...
        $.getTableData = function () {
            if (
                inputCheck($("#titleSelect").val()) &&
                inputCheck($("#shortContentSelect").val()) &&
                inputCheck($("#urlSelect").val())
            )
                $.post("${pageContext.request.contextPath}/insuranceNews/selectLike",
                    {
                        title: $("#titleSelect").val(),
                        shortContent: $("#shortContentSelect").val(),
                        url: $("#urlSelect").val(),
                        releaseTimeFrom: $("#releaseTimeFromSelect").val(),
                        releaseTimeTo: $("#releaseTimeToSelect").val(),
        ...
        $.setStaMap = function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i].createTime == null) continue;
        ...
        $.appendTable = function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                $("tbody tr:last-child").append("<td><textarea rows=\"3\" cols=\"10\" disabled=\"true\">" + data[i].title + "</textarea></td>");
                $("tbody tr:last-child").append("<td><textarea rows=\"3\" cols=\"33\" disabled=\"true\">" + data[i].shortContent + "</textarea></td>");
                $("tbody tr:last-child").append("<td><textarea rows=\"3\" cols=\"10\" disabled=\"true\">" + data[i].url + "</textarea></td>");
                if (data[i].releaseTime != null)
                    $("tbody tr:last-child").append("<td>" + data[i].releaseTime.year + "-" + data[i].releaseTime.monthValue + "-" + data[i].releaseTime.dayOfMonth + "</td>");
                else
            ...
            // console.log("companyArray is " + JSON.stringify(companyArray));
        }
...
///////////////////////////////页标操作结束///////////////////////////////
        ...
        $(document).on("click", ".updateById", function () {
            ...
            $.get("${pageContext.request.contextPath}/insuranceNews/selectById?id=" + id, function (data, status) {
                $("#titleUpdate").val(data.title);
                $("#shortContentUpdate").val(data.shortContent);
                $("#urlUpdate").val(data.url);
                let day = (String(data.releaseTime.dayOfMonth).length < 2) ? ("0" + data.releaseTime.dayOfMonth) : (data.releaseTime.dayOfMonth);
                let month = (String(data.releaseTime.monthValue).length < 2) ? ("0" + data.releaseTime.monthValue) : (data.releaseTime.monthValue);
                let year = data.releaseTime.year;
                $("#releaseTimeUpdate").val(year + "-" + month + "-" + day);
            });
        });
        $(document).on("click", "#submitUpdate", function () {
            console.info("id selected to update is " + id);
            if (
                inputCheck($("#titleUpdate").val()) &&
                inputCheck($("#shortContentUpdate").val()) &&
                inputCheck($("#urlUpdate").val())
            ) {
                $.post("${pageContext.request.contextPath}/insuranceNews/updateAjax",
                    {
                        id: id,
                        title: $("#titleUpdate").val(),
                        shortContent: $("#shortContentUpdate").val(),
                        url: $("#urlUpdate").val(),
                        releaseTime: $("#releaseTimeUpdate").val()
                    },
                    function (data, status) {
                        if (data == true) {
                            alert("更改成功");
        ...
        $(document).on("click", ".deleteById", function () {
            ...
            if (confirm("确认要删除吗？")) {
                ...
                $.post("${pageContext.request.contextPath}/insuranceNews/deleteById",
                    {
                        id: id
                    },
                    function (data, status) {
        ...
        $(document).on("click", "#submitAdd", function () {
            if (
                inputCheck($("#titleAdd").val()) &&
                inputCheck($("#shortContentAdd").val()) &&
                inputCheck($("#urlAdd").val())
            ) {
                $.post("${pageContext.request.contextPath}/insuranceNews/addData",
                    {
                        title: $("#titleAdd").val(),
                        shortContent: $("#shortContentAdd").val(),
                        url: $("#urlAdd").val(),
                        releaseTime: $("#releaseTimeAdd").val(),
                        // createTime:$("#createTimeAdd").val()
                    },
                    function (data, status) {
                        if (data == true) {
                            alert("添加成功");
        ...
        $(document).on("click", "#exportData", function () {
            window.location.href = "${pageContext.request.contextPath}/insuranceNews/exportData"
        });
